...
from app.profile_modeling.home_recommendation import calculate_home_recommendation
...
@router.post("/recommend-home-price", response_model=HomeRecommendationResponse)
def post_recommend_home_price(req: HomeRecommendationRequest) -> HomeRecommendationResponse:
    """
    Calculate recommended home price based on income and financial profile.
    
    Returns safe, recommended, and maximum home prices based on:
    - 28% rule (recommended): Housing ≤ 28% of gross income
    - 35% rule (maximum): Can stretch to 35% but risky
    - Safe range: 20-28% of gross income
    """
    result = calculate_home_recommendation(
        ...
        insurance_pct=req.insurance_pct,
        hoa_monthly=req.hoa_monthly,
    )
    return HomeRecommendationResponse(
        recommended_price=result.recommended_price,
        maximum_price=result.maximum_price,
        safe_min_price=result.safe_min_price,
        safe_max_price=result.safe_max_price,
        monthly_payment_at_recommended=result.monthly_payment_at_recommended,
        monthly_payment_at_maximum=result.monthly_payment_at_maximum,
...
def post_scenario(req: ScenarioCreate, db: Session = Depends(get_db)) -> ScenarioResponse:
    ...
    sanitized_name = html.escape(req.name)
    logger.info(f"Creating scenario: {sanitized_name}")
...
def get_scenarios_list(skip: int = 0, limit: int = 50, db: Session = Depends(get_db)) -> list[ScenarioResponse]:
    ...
    if limit > settings.max_scenarios_per_query:
        raise HTTPException(
            status_code=400,
            detail=f"Limit cannot exceed {settings.max_scenarios_per_query}"
        )
    if skip < 0 or limit < 1:
        raise HTTPException(status_code=400, detail="Invalid pagination parameters")
    
    logger.info(f"Listing scenarios: skip={skip}, limit={limit}")
...
def put_scenario(scenario_id: int, req: ScenarioUpdate, db: Session = Depends(get_db)) -> ScenarioResponse:
    ...
    if s.down_payment >= s.home_value:
        db.rollback()
        raise HTTPException(status_code=400, detail="Down payment must be less than home value.")
    return ScenarioResponse.model_validate(s)
