...
def amortization_schedule(
    ...
    Note: The last payment forces principal to equal remaining balance to ensure the loan
    is paid off exactly, which may cause the final payment to differ from other payments
    due to cumulative rounding effects throughout the schedule.
    """
    if term_years <= 0 or loan_amount <= 0:
        return []
    ...
    for month in range(1, int(n) + 1):
        ...
        if month == int(n):
            principal = balance  # last month: pay off remainder exactly
