...
export function ltv(loanAmount: number, homeValue: number): number {
  ...
  const ratio = loanAmount / homeValue;
  return Math.round(ratio * 10000) / 10000; // 4 decimal precision for ratios
}
...
export function amortizationSchedule(
  ...
  for (let month = 1; month <= totalMonths; month++) {
    ...
    // Note: Last payment forces principal to equal remaining balance to ensure
    // the loan is paid off exactly. This may cause the final payment to differ
    // from other payments due to cumulative rounding effects.
